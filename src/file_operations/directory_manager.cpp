...
{ //
    ...
    if (startsWithSlash)
    {
        ...
        {
            if (outParentInodeId)
                *outParentInodeId = rootDirInodeId; // 根目录的父目录也是根目录（特殊情况）
            if (outLastName)
                *outLastName = "/"; // 或者 "."
            return rootDirInodeId;
        }
        // 跳过开头的 '/'
        if (segment_stream.peek() == '/')
            segment_stream.ignore();
    }
    ...
    if (segments.empty())
    { // e.g. path is "/" or "." or "" relative
        if (path == "/")
        {
            if (outParentInodeId)
                *outParentInodeId = rootDirInodeId; // or some other convention for root's parent
            if (outLastName)
                *outLastName = "."; // Or "/"
            return rootDirInodeId;
        }
        ...
        {
            if (outParentInodeId)
                *outParentInodeId = currentDirInodeId; // or its actual parent if needed
            if (outLastName)
                *outLastName = ".";
        ...
        if (path == "..")
        {
            ...
            if (!inode_manager_->readInode(currentDirInodeId, currentInodeObj))
                return INVALID_INODE_ID;                          //
        ...
        if (outParentInodeId)
            *outParentInodeId = parentId; // this might need careful thought
        if (outLastName && !path.empty())
            *outLastName = path;
        else if (outLastName)
            *outLastName = ".";
...
// --- DEBUG START: Log new entry details before writing ---
    ...
    { //
        if (parentDirInode.direct_blocks[i] == INVALID_BLOCK_ID)
            continue; //
...
{                                               //
    ...
    for (int i = 0; i < NUM_DIRECT_BLOCKS; ++i)
        newDirInode.direct_blocks[i] = INVALID_BLOCK_ID;  //
