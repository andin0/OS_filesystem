...
int FileManager::createFileInode(short ownerUid, short permissions) { //
    ...
    for(int i=0; i < NUM_DIRECT_BLOCKS; ++i) newFileInode.direct_blocks[i] = INVALID_BLOCK_ID; //
...
int FileManager::openFile(int inodeId, OpenMode mode, //
    ...
    if (system_idx == -1) { // Not found in system table, create new entry
        // Find a free slot in systemOpenFileTable or add a new one
        int free_sys_slot = -1;
        for (size_t i = 0; i < systemOpenFileTable.size(); ++i) {
            if (systemOpenFileTable[i].inode_id == INVALID_INODE_ID) { // Assuming INVALID_INODE_ID marks a free slot
                free_sys_slot = i;
                break;
            }
        }
        if (free_sys_slot == -1) { // No free slot, add to end
            if (systemOpenFileTable.size() < MAX_SYSTEM_OPEN_FILES) { // MAX_SYSTEM_OPEN_FILES (define this)
    ...
    if (mode == OpenMode::MODE_WRITE) { //
        ...
        if (!inode_manager_->writeInode(inodeId, systemOpenFileTable[system_idx].inode_cache)) { //
            ...
            systemOpenFileTable[system_idx].open_count--; //
            if (systemOpenFileTable[system_idx].open_count == 0) systemOpenFileTable[system_idx].inode_id = INVALID_INODE_ID; //
            return -1;
        }
    }
...
int FileManager::readFile(int fd, char *buffer, int length, //
                          ...
                          std::vector<SystemOpenFileEntry> &systemOpenFileTable) { // System table is non-const because inode_cache (access time) might be updated. //
