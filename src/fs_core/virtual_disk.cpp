...
VirtualDisk::VirtualDisk(const std::string &diskFilePath, long long diskSize)
    ...
    if (diskFile.is_open()) {
        ...
        if (existingSize > 0 && existingSize >= blockSize_) { // 假设 blockSize_ 至少是有效的
            diskSize_ = existingSize; // 使用现有文件大小
            totalBlocks_ = diskSize_ / blockSize_;
        } else if (existingSize == 0 && diskSize > 0) { // 文件存在但为空，或大小无效
             totalBlocks_ = diskSize_ / blockSize_;
        }
...
bool VirtualDisk::createDiskFile() {
    ...
    if (totalBlocks_ == 0 && diskSize_ > 0) { // 避免除以0后 totalBlocks_ 仍为0的情况
