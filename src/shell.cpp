...
void Shell::handleCd(const std::vector<std::string> &args){
    if (tokens.size() < 2) {
        ...
        if (!fs_->chdir(tokens[1])) {
            std::cerr << "cd: Failed to change directory to " << tokens[1] << std::endl;
        }
    }
}

void Shell::handleLs(const std::vector<std::string> &args){
    std::string path = (tokens.size() > 1) ? tokens[1] : "."; // 默认为当前目录
...
void Shell::handleCreate(const std::vector<std::string> &args){
    if (tokens.size() < 2) {
        ...
        if (!fs_->create(tokens[1])) { //
            std::cerr << "create: Failed to create file " << tokens[1] << std::endl;
        }
    }
}
...
void Shell::handleOpen(const std::vector<std::string>& args) {
    ...
    if (mode_str == "r") mode = OpenMode::MODE_READ;
    else if (mode_str == "w") mode = OpenMode::MODE_WRITE;
    else if (mode_str == "rw") mode = OpenMode::MODE_READ_WRITE;
    else if (mode_str == "a") mode = OpenMode::MODE_APPEND;
    else {
...
void Shell::handleRead(const std::vector<std::string>& args) {
    ...
    try {
        ...
        if (length > 1024 * 10) { // 一个任意的限制，防止过大的缓冲区
...
void Shell::handleWrite(const std::vector<std::string>& args) {
    ...
    try {
        ...
        if (bytes_written >= 0) { // write 可能返回0表示未写入任何内容但非错误
